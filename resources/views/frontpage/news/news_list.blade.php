                ...
                <div class="ec-blog-inner">
                    <div class="ec-blog-image">
                        ...
                        <a href="{{ route('news_detail', $row->slug) }}">
                            <img class="blog-image" src="{{ img_src($row->data_file, 'news') }}" alt="{{ session()->get('locale') == 'id' ? $row->title : $row->title_an }}"/>
                        </a>
                    ...
                    <div class="ec-blog-content">
                        ...
                        <h5 class="ec-blog-title m-0"><a href="{{ route('news_detail', $row->slug) }}">{{ session()->get('locale') == 'id' ? $row->title : $row->title_an }}</a></h5>
                        <div class="ec-blog-date">
                            {{ date('M d, Y', strtotime($row->created_at)) }}</div>
                        <div class="ec-blog-desc">
                            {!! strip_tags(substr(session()->get('locale') == 'id' ? $row->description : $row->description_an, 0, 190)) !!} {!!  strlen(session()->get('locale') == 'id' ? $row->description : $row->description_an) > 190 ? '...' : '' !!}
                        </div>
